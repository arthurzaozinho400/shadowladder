...
export async function loadStats() {
  ...
  const ranking = players.map(p => ({
    nick: p.nick, points: getTierPoints(p.results), user_id: p.user_id,
...
export async function loadQueues() {
  ...
  return data.map(q => ({
    ...
    players: typeof q.players === 'string' ? JSON.parse(q.players) : (q.players || []),
    testers: typeof q.testers === 'string' ? JSON.parse(q.testers) : (q.testers || []),
  }))
}
